...
`mneme/` (backend) and `sidecar/` (inference) are implemented and run end to end on the
Orin against the live camera. `web/` does not exist yet — the frontend and LINE bot are
still unwritten, and every `/api/*` endpoint they need is up and serving real data.
`docs/` remains the contract: treat it as the spec, not as background reading.
...
`spec.md` v1.6, `backend.md` v1.5, `sidecar.md` v1.6.
...
- **The camera is CSI, not USB**: an IMX219 emitting 10-bit Bayer (`RG10`), which
  `cv2.VideoCapture` cannot decode. Frames come in through `--camera-cmd` (`spec.md` §7)
  from a GStreamer pipeline; the sensor floor is 21fps so throttle with `videorate`, and
  use `multifilesink` — `num-buffers=1` with `filesink` hangs without flushing.
  A killed pipeline leaves the Argus session wedged (`Failed to create CaptureSession`);
  `sudo systemctl restart nvargus-daemon` clears it. **Worth knowing on demo day.**
...
web/              # static frontend, served at / — NOT WRITTEN YET (app.py already mounts it)
...
# 3. the app, with the CSI camera through GStreamer (cv2 cannot open it)
.venv/bin/python -m mneme --data-dir ./data --sidecar /tmp/vlm.sock --bind 0.0.0.0:8080 \
    --sidecar-timeout-ms 60000 \
    --camera-cmd 'gst-launch-1.0 nvarguscamerasrc ! video/x-raw(memory:NVMM),width=1920,height=1080,framerate=30/1 ! nvvidconv ! video/x-raw,format=I420 ! videorate ! video/x-raw,framerate=1/1 ! jpegenc ! multifilesink location=frame_%05d.jpg'
```
